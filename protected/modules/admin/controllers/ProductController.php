...
class ProductController extends AdminController {
    ...
    public function actionTranslate($id) {
        $this->pageTitle = 'Products / Translate product';

        $errors = array();

        $model = Product::model()->findByPk($id);
        $contentModel = Content::model()->getModuleContent('product', $id, 'ru');
    ...
    public function actionMoveNU($id) {
        ...
        if ($sort > 1) {
            $upperModel = ProductNode::model()->findBySql(
                    "SELECT * FROM product_nodes WHERE product_id = :product_id AND sort < :sort", array(':product_id' => $model->product_id, ':sort' => $sort)
            );
            if ($upperModel) {
                $model->sort = $upperModel->sort;
                $model->save();
    ...
    public function actionMoveND($id) {
        $model = ProductNode::model()->findByPk($id);
        $sort = $model->sort;
        $maxModel = ProductNode::model()->findBySql("SELECT MAX(sort) as sort FROM product_nodes WHERE product_id = :product_id", array(':product_id' => $model->product_id));
        if ($sort < $maxModel->sort) {
            $upperModel = ProductNode::model()->findBySql(
                    "SELECT * FROM product_nodes WHERE product_id = :product_id AND sort > :sort", array(':product_id' => $model->product_id, ':sort' => $sort)
            );
